  ...
  saveFileBody: function (obj, success, error) {
    ...
    $.ajax({
      ...
      success: function (rdata) {
        ...
        if (!obj.tips){
          if(!rdata.status){
            layer.confirm(rdata.msg.substr(0, rdata.msg.length - 1)+'，或选择强制保存。',{icon:2,closeBtn:2,btn:['强制保存','取消']},function(index){
              obj.force = 1
              layer.close(index)
              layer.confirm('<span style="color:#333">检测到文件已被修改，强制保存将会<span style="color:red">覆盖文件内容</span>，请谨慎操作!</span>',{title:"强制保存文件",closeBtn:2,icon:7,btn:['确认','取消']},function(){
                if(obj.ACE['session']){
                ...
                $('.item_tab_' + obj.ACE.id + ' .icon-tool').attr('data-file-state', '0').removeClass('glyphicon-repeat').addClass('glyphicon-remove');
                $('.item_tab_' + obj.ACE.id + ' .icon-tool').addClass('glyphicon-exclamation-sign').removeClass('glyphicon-remove').attr('data-file-state', '1');
              })
            },function(index){
              layer.close(index)
              $('.item_tab_' + obj.ACE.id + ' .icon-tool').attr('data-file-state', '0').removeClass('glyphicon-repeat').addClass('glyphicon-remove');
      ...
      content: '<div class="libLogin" style="padding:20px 30px">\
          <div class="bt-form text-center">\
              ...
              <div class="line mb15" style="display:'+ (!type ? 'none' : 'block') + '">\
                <h3 class="c2 f20 text-center mtb20">'+ (username ? '切换': '绑定') +'宝塔官网账号<a href="javascript:;" class="bind_ps bt-ico-ask">?</a></h3>\
              </div>\
              ...
              <ul class="help-info-text c7" style="text-align: left;"><li >当前'+ (username ? '切换': '绑定') +'的是宝塔官网注册账号,用于授权和购买产品,非面板账号</li></ul>\
          </div >\
      </div > ',
      success: function () {
        var time = '';
