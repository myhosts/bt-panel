...
var bt_file = {
  ...
  event_bind: function () {
    ...
    //   });
    // });
    $('.file_table_view .file_list_content').on('dblclick', '.icon-lock,.dir_details_span', function (e) {
      e.stopPropagation()
    })
    // 获取文件夹大小
    ...
    $.each(data, function (index, item) {
      ...
      _html += '<div class="file_tr" data-index="' + index + '" data-filename="' + item.filename + '" ' + (bt.get_cookie('rank') == 'icon' ? 'title="' + path + '&#13;' + lan.files.file_size + ':' + bt.format_size(item.size) + '&#13;' + lan.files.file_etime + ':' + bt.format_data(item.mtime) + '&#13;' + lan.files.file_auth + ':' + item.user + '&#13;' + lan.files.file_own + ':' + item.root_level + '"' : '') + '>' +
        ...
        '<div class="file_td file_size"><span>' + (item.type == 'dir' ? bt.files.dir_details_span(path,true) : bt.format_size(item.size)) + '</span></div>' +
          ...
          '<div class="file_td file_ps"><span class="file_ps_title" style="width:100%" title="' + item.ps + '">' + (item.is_os_ps ? ('<span>' + item.ps +'</span>') : '<input type="text" class="set_file_ps" data-value="' + item.ps + '" value="' + item.ps + '" />') + '</span></div>' +
          '<div class="file_td file_operation"><div class="set_operation_group" style="width:100%">' +
            ...
            <div class="tab-con">\
              <div class="property-box file_list_content  active">\
                ...
                <div class="attr-box" >\
                  ...
                  <div class="attr-content"><span title="'+ (res.path + '/' + data.filename).replace('//','/') + '">' + (res.path + '/' + data.filename).replace('//','/') + '<i class="ico-copy cursor btcopy ml5 copyProperytPath" data-clipboard-text="' + (res.path + '/' + data.filename).replace('//','/') + '" title="复制文件路径"></i></span></div>\
                </div>\
                <div class="attr-box" >\
    ...
    bt.open({
      ...
      content: '\
			<div class="spyware_detection_view">\
				...
				</div>'+ bt.render_help(['本处为快速扫描，基于木马特征进行识别，可能会有遗漏或误报，更精准的结果以【网站-设置-安全扫描】为主'])
      +'</div>',
      success: function (layero, index) {
        that_layer = this;
        $layer = layero;
