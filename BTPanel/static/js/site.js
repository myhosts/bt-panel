...
var site = {
          ...
          $.post('/deployment?action=GetSiteList', function (rdata) {
            $.post('/site?action=GetPHPVersion', function (res) {
              ...
              function otherConfig(database) {
                ...
                db_html += '<span class="mr10">数据库：'+ (bt.config['mysql'].setup ? database.dataname+'数据库':'<a class="btlink install_database" style="color:#ef0808">未安装数据库，点击安装</a>') +'</span>'
  ...
  adding_pseudo_static_in_bulk: function (_that) {
        ...
        yes: function (indexx, layero) {
          bt.open({
              ...
              function reanderSiteList(collection) {
                if (!collection.length)
                  return $siteList.html(
                    '<div class="text-center" style="height: 100%;font-size: 14px;line-height: 180px;">暂无站点数据</div>'
                  );
                var html = '';
                $.each(collection, function (index, value) {
                  html +=
            ...
            yes: function (index, layero) {
              layer.close(index);
              layer.close(indexx);
              bt_tools.send({url:'site?action=SetRewriteLists', data:{
                ...
                for (
                  ...
                  if (item.status) {
                    html +=
                      '<tr><td>' +
                      item.name +
                      '</td><td><div style="float:right;"><span style="color:#20a53a;">部署成功</span></div></td></tr>';
                  } else {
                    html +=
                      '<tr><td>' +
                      item.name +
                      '</td><td><div style="float:right;"><span style="color:red;">部署失败</span></div></td></tr>';
                  }
                }
    ...
    bt.send('GetRewriteLists', 'site/GetRewriteLists', {
      ...
      if (err_site.length > 0) {
        ...
        for (
          ...
          html +=
            ...
            item.name +
            '</td><td style="text-align: left"><div><span style="color: red">' + item.msg + '</span></div></td></tr>';
        }
        bt.open({
          title: '批量设置站点伪静态',
          btn: ['下一步', '取消'],
          area: ['490px', '400px'],
          content: '<div class="batch_title" style="margin-top: 0">' +
            '<span class>' +
            '<span class="batch_icon" style="background-position:0px 0"></span>' +
            '<span class="batch_text" style="font-size: 17px"> 有' + err_site.length + '个网站无法部署伪静态！如需忽略请点击下一步。</span>' +
            '<div class="' + (length > 4 ? 'fiexd_thead' : '') + ' batch_tabel divtable" style="margin-top: 20px; overflow: auto;height: 108px;">' +
            '<table class="table table-hover" >' +
            '<thead>' +
