      ...
      $reader_content: function (data, page) {
        ...
        function old_pub_content() {
          ...
          do {
            ...
            if (data.length > 0) tbody += '</tr>'
            if (data.length == 0) tbody += '<tr class="no-tr"><td colspan="' + column.length + '" style="text-align:center;">' + (_that.config['default'] || '数据为空') + '</td></tr>';
            i++;
          } while (i < data.length);
        ...
        function pub_content(rdata_header) {
          ...
          if (setting_btn) {
            ...
            var _flag = _that.cache_set_all[_that.config.headerId] && cache_data.filter(function (item, index) {
              if(_that.cache_set_column[index] == undefined){
                return
              }
            ...
            for (var j = 0; j < _that.cache_set_column.length; j++) {
              var obj = _that.cache_set_column[j], title = $(obj.title).text();
            ...
            // flag: true 有width为auto的列
            // 当全部列有设置宽度且显示长度与列数相等时，不进行操作
            if ((flag_num == _that.cache_set_column.length)) {
              if (!flag && column[1]['width'] == 'auto') {
                column[1]['width'] = (column[1]['temp_width'] ? column[1]['temp_width'] : '180')
              }
            } else {
              column[1]['temp_width'] = column[1]['width']
              column[1]['width'] = 'auto'
            }
            //渲染显示隐藏列按钮
              ...
              set_interval = setInterval(function () {
                if (_this.next().css('display') == 'block') {
                  var bW = _this.width(), bH = _this.height(), bT = _this.offset().top, bL = _this.offset().left,
                    uW = _this.next().width(), uH = _this.next().height(), uT = _this.next().offset().top,
            ...
            $(_that.config.el + ' .setting_ul .setting_ul_li').unbind('click').click(function (e) {
              ...
              bt.send('set_table_header', 'config/set_table_header', {
                table_name: _that.config.headerId,
                table_data: JSON.stringify(_that.cache_set_column)
              }, function (res) {
                bt.send('get_config', 'config/get_config', {}, function (rdata_header) {// 获取所有表自定义列数据
                  bt.config = rdata_header;
