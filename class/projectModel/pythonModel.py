...
class main(projectBase):
    ...
    def get_cloud_version(self, get=None):
        ...
        if not res:
            return {"status": False, "msg": "请求不到官方python版本数据，请切换节点试一试.<br>相关教程参考:<br><a>https://www.bt.cn/bbs/thread-87257-1-1.html</a>"}
        text = res.split('\n')
    ...
    def install_pip(self, vpath, pyv):
        ...
        if res.find(vpath) == -1:
            public.ExecShell("rm -rf {}/lib/python3.{}/site-package".format(vpath, _pyv))
    ...
    def __check_args(self, get):
        ...
        test_rule = {
            ...
            "version": lambda x: (x in self._get_python_v(None), 'Python版本未安装'),
    ...
    def _get_project_state(self, project_info):
        ...
        if "check_sh" not in project_info["project_config"]:
            return
    ...
    def AddProjectDomain(self, get):
        ...
        for domain in domains:
            ...
            if len(domain_arr) == 1:
                domain_arr.append(80)
                domain += ':80'
            if not bool(str(domain_arr[1]).strip()):
                domain_arr[1] = 80
                domain = domain_arr[0] + ":80"
