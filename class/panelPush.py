...
class panelPush:
    ...
    def set_push_config(self, get):
        ...
        data = self._get_conf()
        if 'title' in json.loads(get['data']).keys():
            title = 'title'
        else:
            title = 'type'
        public.WriteLog('告警设置', '添加告警任务【{}】'.format(json.loads(get['data'])[title]))
    ...
    def set_push_status(self, get):
        ...
        public.WriteLog('告警设置',
                        "设置任务【{}】的状态为【<span style='color:{}'>{}</span>】".format(data[module][id]['title'],
                                                                                        color, status))
    ...
    def del_push_config(self, get):
        ...
        if id == "panel_login":
            ...
            title = data[module][id]['title']
        public.WriteLog('告警设置', "删除告警任务【{}】".format(title))
        p_list = public.get_modules('class/push')
        if not module in p_list:
            return public.returnMsg(False, '指定模块{}未安装.'.format(module))
        push_module = getattr(p_list[module], module)()
