...
session.save_handler = files'''.format(path, sess_path, sess_path)
    ...
    def upload(self, args):
        ...
        try:
            if self.get_real_len(args.f_name) > 256: return public.returnMsg(False, '文件名长度超过256个字节')
        except:
            pass
...
''' % (get.path, get.shell)
    ...
    def exec_git(self,get):
        ...
        if not re.match(pattern,get.giturl):
            return public.returnMsg(False,'错误的URL地址!')
