...
class main(safeBase):
            ...
            if ports.find(',') != -1:
                ...
                if end_port:
                    if (int(add_start) >= int(start_port) and int(add_start) <= int(end_port)) or (
                        ...
                        return public.returnMsg(False,
                                                '{}端口范围与现有防火墙规则端口{}重叠，请勿重复添加！'.format(ports, i2[
                                                    'ports']))
                else:
                    if int(add_start) <= int(start_port) and int(add_end) >= int(start_port):
                        return public.returnMsg(False,
                                                '{}端口范围包含已添加端口{}，请勿重复添加！'.format(ports, i2['ports']))
            else:
                if end_port:
    ...
    def sync_rules(self):
        ...
        args.source = ""
        args.brief = ""
        if not public.M('firewall_new').where('ports=? and protocol LIKE ?',
                                              (args.ports, '%' + args.protocol + '%')).count():
                            ...
                            if "###" in rule:
                                ...
                                if rule_result:
                                    ...
                                    if r_result[1] == 'any':
                                        ...
                                        protocol = "udp"
                                    if i['ports'] == r_result[2] and protocol in i['protocol'] and i['address'] == \
                                            r_result[-1]:
                                        sum += 1
                        except:
                            continue
    ...
    def modify_rules(self, get):
        ...
        data = public.M('firewall_new').where('id=?', (id,)).field(
            'id,address,protocol,ports,types,brief,addtime,domain').find()
        if data:
        ...
        public.M('firewall_new').where('id=?', id).update(
            {'address': address, 'protocol': protocol, 'ports': ports, 'types': types, 'brief': brief,
             'addtime': addtime, 'sid': sid,
             'domain': domain})
        if domain:
    ...
    def handle_firewall_ip(self, address, types):
        ...
        for ip in ip_list:
            ...
            entry.text = ip
            root.append(
                entry)  # public.ExecShell('firewall-cmd --permanent --zone=public --ipset='+ address + ' --add-entry='+ip)
        self.format(root)
    ...
    def create_ip_rules(self, get):
        ...
        if result: return result
        for address in address_list:
            if public.M('firewall_ip').where("address=? and types=? and domain=?",
                                             (address, types, domain)).count() > 0:
    ...
    def import_rules(self, get):
        try:
            ...
            if not_pay_list:
                ...
                return public.ReturnMsg(result["status"],
                                        "{}<br/>指定域名功能为企业版专享，以下规则未导入：<br/>{}".format(result["msg"],
                                                                                                        not_pay_list))
...
class Sqlite():
    ...
    def create_table(self):
        ...
        create_table_str = public.M('sqlite_master').where('type=? AND name=?', ('table', 'firewall_new')).getField(
            'sql')
        ...
        create_table_str = public.M('sqlite_master').where('type=? AND name=?', ('table', 'firewall_ip')).getField(
            'sql')
        if 'sid' not in create_table_str:
