...
class panelWarning:
    ...
    def get_vuln_result(self):
        ...
        if not os.path.exists(self.__vul_list):
            ...
            vul_num = len(self.get_vul_list())
        if not os.path.exists(self._vuln_result):
            tmp_dict = {"vul_count": vul_num, "risk": [], "ignore": [],
    ...
    def compare_md5(self):
        ...
        if not os.path.exists(self.__vul_list):
            try:
                public.downloadFile("https://download.bt.cn/install/src/high_risk_vul.zip", self.__path + "/high_risk_vul.zip")
