...
class system:
    ...
    def ServiceAdmin(self,get=None):
        #服务管理
        type_list=["stop","start","restart","reload","kill"]
        if not get.type in type_list: return public.returnMsg(False,'EXEC_ERR_TASK')
        if not re.match('^[a-zA-Z0-9\-\_]+$',get.name):
            return public.returnMsg(False,'参数：{}，要求只能是英文字母或数据组成'.format(get.name))
        if get.name == 'apache':get.name = 'httpd'
        # if not os.path.exists("/etc/init.d/"+get.name):  # 部分软件管理init不存在
        #     return public.returnMsg(False,'INIT_EXEC_ERR_NOT',(get.name,))
